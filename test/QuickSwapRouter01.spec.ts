  ...
  for (const routerVersion of Object.keys(RouterVersion)) {
    ...
    describe(routerVersion, () => {
      ...
      it('removeLiquidityETH', async () => {
        ...
        await tokenQLC.transfer(WETHPair.address, tokenQLCAmount)
        expect(await tokenQLC.balanceOf(WETHPair.address)).to.eq(tokenQLCAmount)
        await WETHPair.distributeQLC()
        ...
        const totalSupplyTokenQLC = await tokenQLC.totalSupply()
        expect(await tokenQLC.balanceOf(WETHPair.address)).to.eq(MINIMUM_LIQUIDITY.add(expandTo18Decimals(2)))
      ...
      describe('swapExactTokensForTokens', () => {
        ...
        it('gas', async () => {
          ...
          expect(receipt.gasUsed).to.eq(
            {
              // [RouterVersion.UniswapV2Router01]: 125842,
              [RouterVersion.UniswapV2Router02]: 191193
            }[routerVersion as RouterVersion]
          )
        }).retries(3)
      })
      ...
      describe('swapExactETHForTokens', () => {
        ...
        it('happy path', async () => {
          const WETHPairToken0 = await WETHPair.token0()
          await expect(
            router.swapExactETHForTokens(0, [WETH.address, WETHPartner.address], wallet.address, MaxUint256, true, {
              ...overrides,
              value: swapAmount
            })
          )
        ...
        it('gas', async () => {
          ...
          expect(receipt.gasUsed).to.eq(
            {
              // [RouterVersion.UniswapV2Router01]: 162750,
              [RouterVersion.UniswapV2Router02]: 228013
            }[routerVersion as RouterVersion]
          )
        }).retries(3)
      })

...
export async function v2Fixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<V2Fixture> {
  ...
  const tokenQLC = await deployContract(wallet, ERC20, [expandTo18Decimals(10000)])
  ...
  // deploy V2
  const factoryV2 = await deployContract(wallet, UniswapV2Factory, [wallet.address, tokenQLC.address], overrides)
  ...
  const stoken = new Contract(pair.stoken(), JSON.stringify(ERC20.abi), provider).connect(wallet)
  ...
  await factoryV2.createPair(token0.address, tokenQLC.address)

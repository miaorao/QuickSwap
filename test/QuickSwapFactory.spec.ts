...
describe('UniswapV2Factory', () => {
  ...
  async function createPair(tokens: [string, string]) {
    ...
    await expect(factory.connect(other).createPair(...tokens)).to.be.reverted // QuickSwap: NOT MANAGER
    await expect(factory.createPair(...tokens)).to.be.reverted // UniswapV2: PAIR_EXISTS
    await expect(factory.createPair(...tokens.reverse())).to.be.reverted // UniswapV2: PAIR_EXISTS
    expect(await factory.getPair(...tokens)).to.eq(create2Address)
    expect(await factory.getPair(...tokens.reverse())).to.eq(create2Address)
  ...
  it('createPair:gas', async () => {
    ...
    expect(receipt.gasUsed).to.eq(4631443)
  })

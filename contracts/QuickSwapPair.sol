...
import './QuickSwapSlippageToken.sol';
...
contract QuickSwapPair is QuickSwapERC20 {
    ...
    QuickSwapSlippageToken public stoken;

    uint128 private token0Slippage;
    uint128 private token1Slippage;
    ...
    function getSlippageAmount() public view returns (uint128 _token0Slippage, uint128 _token1Slippage) {
        _token0Slippage = token0Slippage;
        _token1Slippage = token1Slippage;
    }
    ...
    function _safeTransfer(address token, address to, uint256 value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'QuickSwap: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'QuickSwap: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
        stoken = new QuickSwapSlippageToken(0);
    }

    // update reserves and, on the first call per block, price accumulators
    function _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'QuickSwap: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IQuickSwapFactory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        if (_totalSupply == 0) {
            address migrator = IQuickSwapFactory(factory).migrator();
            if (msg.sender == migrator) {
                liquidity = IMigrator(migrator).desiredLiquidity();
                require(liquidity > 0 && liquidity != uint256(-1), "QuickSwap: Bad desired liquidity");
            } else {
                require(migrator == address(0), "QuickSwap: Must not have migrator");
        ...
        require(liquidity > 0, 'QuickSwap: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint256 amount0, uint256 amount1) {
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        // (uint256 _qlcForLiquidityProvider, ) = getQLCAmount();
        address _token0 = token0; // gas savings
        address _token1 = token1; // gas savings
        // address _tokenQLC = IQuickSwapFactory(factory).tokenQLC();
        ...
        bool feeOn = _mintFee(_reserve0, _reserve1);
        {
            ...
            // amount2 = liquidity.mul(_qlcForLiquidityProvider) / _totalSupply;
            require(amount0 > 0 && amount1 > 0, 'QuickSwap: INSUFFICIENT_LIQUIDITY_BURNED');
            _burn(address(this), liquidity);
        }
        _safeTransfer(_token0, to, amount0);
        _safeTransfer(_token1, to, amount1);
        // if (amount2 > 0) {
        //     _qlcForLiquidityProvider = _qlcForLiquidityProvider.sub(amount2);
        //     _safeTransfer(_tokenQLC, to, amount2);
        // }
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this));

        // qlcForLiquidityProvider = uint128(_qlcForLiquidityProvider);
        _update(balance0, balance1, _reserve0, _reserve1);
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'QuickSwap: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'QuickSwap: INSUFFICIENT_LIQUIDITY');
        ...
        uint256 balance1;
        {
            ...
            require(to != _token0 && to != _token1, 'QuickSwap: INVALID_TO');
            if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
            if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
            if (data.length > 0) IQuickSwapCallee(to).QuickSwapCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
        }
        uint256 amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0;
        uint256 amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, 'QuickSwap: INSUFFICIENT_INPUT_AMOUNT');
        {
    ...
    function dealSlippageWithIn(address[] calldata path, uint256 amountIn, address to, bool ifmint) external lock returns (uint256 amountOut) {
        ...
        // avoids stack too deep errors
        {
            ...
            require(amount >= amountIn, 'QuickSwap: Invalid Amount');
        }
        ...
        if (amountOutReal > amountOutMarket) {
            ...
            amountOut = amountOutReal.sub(halfSlippage);
            if (path[1] == _token0) {
                token0Slippage = uint128(_token0Slippage.add(halfSlippage));
            } else {
                token1Slippage = uint128(_token1Slippage.add(halfSlippage));
            }
        }
    ...
    function dealSlippageWithOut(address[] calldata path, uint256 amountOut, address to, bool ifmint) external lock returns (uint256 extra) {
        ...
        if (amountInReal < amountInMarket) {
            ...
            if (path[0] == _token0) {
                token0Slippage = uint128(_token0Slippage.add(extra));
            } else {
                token1Slippage = uint128(_token1Slippage.add(extra));
            }
        }
